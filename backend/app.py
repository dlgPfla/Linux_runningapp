...
COURSE_ROUTES_CSV = os.path.join(DATA_DIR, "course_routes.csv")
...
def number_or_none(value):
    """숫자로 변환할 수 없는 값은 API에서 null로 내려보낸다."""
    if pd.isna(value):
        return None
...
def load_course_routes() -> pd.DataFrame:
    """
    course_routes.csv를 읽어 지도 Polyline 표시용 좌표 DataFrame으로 반환.
    필수 값이 없거나 숫자로 변환할 수 없는 행은 제외한다.
    """
    if not os.path.exists(COURSE_ROUTES_CSV):
        raise FileNotFoundError(COURSE_ROUTES_CSV)

    required = ["course_id", "point_order", "latitude", "longitude"]
    df = pd.read_csv(COURSE_ROUTES_CSV)
    print("course_routes rows:", len(df))

    missing = [col for col in required if col not in df.columns]
    if missing:
        raise ValueError(f"course_routes.csv missing columns: {', '.join(missing)}")

    df = df[required].copy()
    df["course_id"] = df["course_id"].where(pd.notnull(df["course_id"]), "").astype(str).str.strip()
    df["point_order"] = pd.to_numeric(df["point_order"], errors="coerce")
    df["latitude"] = pd.to_numeric(df["latitude"], errors="coerce")
    df["longitude"] = pd.to_numeric(df["longitude"], errors="coerce")

    df = df[df["course_id"] != ""]
    df = df.dropna(subset=["point_order", "latitude", "longitude"])
    print("valid route rows:", len(df))
    df["point_order"] = df["point_order"].astype(int)

    return df.sort_values(["course_id", "point_order"])
...
def course_routes_to_list(df: pd.DataFrame):
    """course_id별로 좌표를 묶어 API 응답 구조로 변환."""
    routes = []

    for course_id, group in df.groupby("course_id", sort=False):
        points = [
            {
                "point_order": int(row.get("point_order")),
                "latitude": float(row.get("latitude")),
                "longitude": float(row.get("longitude")),
            }
            for _, row in group.iterrows()
        ]
        routes.append({"course_id": course_id, "points": points})
...
# ── GET /api/course-routes ───────────────────────────────────────────────────
@app.route("/api/course-routes", methods=["GET"])
def get_course_routes():
    """대표 러닝 코스 Polyline 표시용 좌표 데이터 반환."""
    routes_exists = os.path.exists(COURSE_ROUTES_CSV)
    print("course_routes exists:", routes_exists)

    if not routes_exists:
        return jsonify({
            "total": 0,
            "routes": [],
            "message": "course_routes.csv not found",
        })

    try:
        df = load_course_routes()
        routes = course_routes_to_list(df)
        print("route groups:", len(routes))
        ...
        return jsonify({"error": str(exc), "total": 0, "routes": []}), 500
